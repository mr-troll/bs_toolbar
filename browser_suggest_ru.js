...
var browser_toolbar_init=function(){
	...
	if(BrowserDetect.browser=='Explorer' || stable_browsers[BrowserDetect.browser]<=BrowserDetect.version){
		...
		if(windows=='xp' || windows=='2000'){
			toolbar.innerHTML='<span style="display:inline-block;height:30px;">Ваш браузер, '+ie_icon+BrowserDetect.browser+" "+BrowserDetect.version+' устарел! Он не даст быстроту современных браузеров. Пожалуйта скачайте один из них: '+chrome_icon+firefox_icon+ opera_icon+'</span>';
		}
		else if(windows=='seven' || windows=='vista'){
  			toolbar.innerHTML='<span style="display:inline-block;height:30px;">Обновите ваш браузер, '+ie_icon+BrowserDetect.browser+" "+BrowserDetect.version+', до версии '+stable_browsers[BrowserDetect.browser]+'. Вы можете запустить Windows Update или скачать его с <a href="http://www.microsoft.com/en-us/download/default.aspx">сайта Microsoft</a>. Либо: '+chrome_icon+	firefox_icon+ opera_icon+'</span>';
		}
	}
